...
namespace LibraryAPI.Repositories {
    public class RecommendationsRepository : IRecommendationsRepository {
        ...
        public IEnumerable<BookDTO> GetRecommendationsForUser (int userId) {
            ...
            var user = (from u in _db.Users where u.Id == userId &&
                u.Deleted == false select new UserDTO {
                    Id = u.Id,
                        ...
                        LoanHistory = (from l in _db.Loans join b in _db.Books on l.BookId equals b.Id where l.UserId == userId &&
                            b.Deleted == false select new LoanDTO {
                                Id = l.Id,
                                    BookTitle = b.Title,
                                    LoanDate = l.LoanDate,
                                    EndDate = l.EndDate
            ...
            if (user.LoanHistory != null) {
                ...
                foreach (var book in user.LoanHistory) {
                    ...
                    var author = (from b in _db.Books where b.Title == book.BookTitle &&
                        b.Deleted == false select b.Author).SingleOrDefault ();
                    authors.Add (author);
                }
                ...
                foreach (var author in authors) {
                    var recommendedBooks = (from b in _db.Books where b.Author == author &&
        ...
        private List<BookDTO> GetRandomRecommendations (List<string> titles) {
            List<BookDTO> recommendations = new List<BookDTO> ();
            var books = (from b in _db.Books where b.Deleted == false select new BookDTO {
                Id = b.Id,
                    Title = b.Title,
                    Author = b.Author,
                    ReleaseDate = b.ReleaseDate,
